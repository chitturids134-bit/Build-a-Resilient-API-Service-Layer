...
export default function ProfilePage() {
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true)
    try {
      const res = await userService.updateUser(1, form)
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 border border-gray-200 rounded-xl p-6">
          ...
          {saveMsg && (
            <div className={`text-sm rounded-lg p-3 mb-4 ${saveMsg.includes('saved') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700 border border-red-200'}`}>
              {saveMsg}
            </div>
          )}
