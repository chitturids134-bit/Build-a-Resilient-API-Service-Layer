...
import { productsService, cartService, userService } from '../services/api'
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    productsService.getProductById(id)
      .then(res => {
        ...
        setProduct(enriched)
        return productsService.getProductsByCategory(enriched.category)
      })
      .then(res => {
      ...
      .catch(err => {
        setError(err.message === 'Request failed with status code 404' ? 'Product not found' : err.message)
        setLoading(false)
      })
  }, [id])
  ...
  const handleReview = async (e) => {
    ...
    try {
      await userService.createUser({ productId: id, rating: 5 })
